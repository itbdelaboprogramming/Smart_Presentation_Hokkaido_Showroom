...
try {
    $stmt = $pdo->prepare("SELECT * FROM products WHERE category = 'crushing' OR product_key = 'Hokkaido Crushing Full Plant' ORDER BY id ASC");
    $stmt->execute();
    ...
    foreach ($products as $p) {
        ...
        // Format same with data.json structure
        $formattedData[$key] = [
            "id" => $p['id'], 
            "info" => $p['info'],
            "glb_file" => $p['glb_file'],
...
<html lang="en">
    <head>
        <title>スマート・プレゼンテーション - 破砕プラント</title>
        ...
        <link rel="stylesheet" href="./style/admin-ui.css"> <script async src="https://unpkg.com/es-module-shims@1.6.3/dist/es-module-shims.js"></script>
        ...
        <script>
            ...
            console.log("Data Loaded from DB:", window.PRODUCT_DATA);
        </script>
            ...
            <div class="catalogue-container-2" id="catalogue-container-2" style="display:flex;">
                ...
                <div class="catalogue-description-2">
                    ...
                    foreach ($productsList as $index => $prod): 
                        $isActive = ($prod['product_key'] == 'Hokkaido Crushing Full Plant') ? 'active' : '';
                        $displayName = $prod['product_key']; 
                        if ($displayName == 'Hokkaido Crushing Full Plant') $displayName = 'Overview';
                    ?>
    ...
    <?php if ($isAdmin): ?>
    <div class="admin-modal-overlay hidden" id="admin-annotation-modal" style="z-index: 9999;">
        <div class="admin-modal-content" style="max-width: 600px; background-color: #344054; color: white; border-radius: 10px; padding: 20px;">
            
            <div class="admin-modal-header" style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #475467; padding-bottom: 10px; margin-bottom: 15px;">
                <h2 style="margin: 0; color: #74e7d4;">Manage Annotations</h2>
                <button class="admin-modal-close" id="admin-anno-close-btn" style="background: none; border: none; color: #98a2b3; font-size: 24px; cursor: pointer;">&times;</button>
            </div>

            <div class="admin-modal-body">
                <p id="anno-product-name" style="color: #74e7d4; margin-bottom: 15px; font-weight: bold;"></p>
                <input type="hidden" id="anno-product-id">

                <button class="admin-button-add" id="btn-add-annotation-mode" style="width: 100%; margin-bottom: 15px; background-color: #e67e22; color: white; border: none; padding: 10px; border-radius: 5px; cursor: pointer;">
                    + Add New Annotation (Click 3D Model)
                </button>

                <div style="max-height: 300px; overflow-y: auto; background: #1d2538; border-radius: 5px; padding: 10px; border: 1px solid #475467;">
                    <table style="width: 100%; color: white; border-collapse: collapse;">
                        <thead>
                            <tr style="border-bottom: 1px solid #475467;">
                                <th style="text-align: left; padding: 8px; color: #98a2b3;">Text</th>
                                <th style="text-align: right; padding: 8px; color: #98a2b3;">Action</th>
                            </tr>
                        </thead>
                        <tbody id="annotation-list-body">
                            </tbody>
                    </table>
                </div>
            </div>

            <!-- <div class="admin-modal-footer" style="margin-top: 20px; text-align: right;">
                <button type="button" id="admin-anno-cancel-btn" style="background-color: #475467; color: white; border: none; padding: 8px 16px; border-radius: 5px; cursor: pointer;">Close</button>
            </div> -->
        </div>
    </div>

    <script src="./js/annotation-admin.js"></script>
    <?php endif; ?>
    </body>
</html>